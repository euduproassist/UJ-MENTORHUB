  ...
  function seedIfEmpty() {
    ...
    if (!users) {
      const sampleTutors = [
        ...
        { id: uid("u-"), role: "tutor", name: "Bongani K", email: "bongani@uj.ac.za", suspended: false },
        { id: uid("u-"), role: "tutor", name: "Chen L", email: "chen@uj.ac.za", suspended: false },
      ];
...
function generateReport(type, from, to) {
  switch(type) {
    ...
    case "logins":
      return generateLoginReport(from, to);

    case "ratings":
      return generateRatingsReport(from, to);

    case "activity":
      return generateActivityReport(from, to);

    case "needHelpNow":
      return generateNeedHelpNowReport(from, to);

    case "anonymous":
      return generateAnonymousReport(from, to);

    case "requests":
    default:
      return generateRequestsReport(from, to);
  }
}
function generateLoginReport(from, to) {
  const data = backend.getLoginLogs(from, to);

  let html = `
    <h3>Total Logins Report</h3>
    <table border="1">
      <tr>
        <th>Name</th>
        <th>Role</th>
...
function generateRequestsReport(from, to) {
  const data = backend.getRequestsReport(from, to);

  let html = `
    <h3>Tutoring & Counselling Requests</h3>
    <table border="1">
      <tr>
        <th>Student</th>
        <th>To</th>
...
function generateRatingsReport(from, to) {
  const {positive, negative} = backend.getRatings(from, to);

  let html = `<h3>Ratings & Comments</h3>`;

  html += `<h4 style="color:green">Positive</h4>`;
  html += `<table border="1">
    <tr><th>Student</th><th>To</th><th>Rating</th><th>Comment</th><th>Date</th></tr>
  `;
  positive.forEach(p => {
    html += `<tr>
      <td>${p.from}</td>
  ...
  html += `<h4 style="color:red">Negative</h4>`;
  html += `<table border="1">
    <tr><th>Student</th><th>To</th><th>Rating</th><th>Comment</th><th>Date</th></tr>
  `;
  negative.forEach(n => {
...
function generateActivityReport(from, to) {
  const data = backend.getActivityHours(from, to);

  let html = `<h3>Activity Hours</h3>
    <table border="1">
      <tr>
        <th>Name</th>
        <th>Role</th>
...
function generateNeedHelpNowReport(from, to) {
  const data = backend.getNeedHelpNow(from, to);
...
function generateAnonymousReport(from, to) {
  const data = backend.getAnonymous(from, to);
